''' 
LOADERS V2 - Adapté pour Model V5_03_C
...
def load_disciplines_v2() -> List[discipline]:
    """
...
def load_eleves_v2(eleves_path: Path) -> List[eleve]:
    """
...
def load_stages_v2(stages_path: Path) -> Dict[Tuple[str, int], List[Dict]]:
    """
...
def load_calendars_v2(data_dir: Path) -> Dict[niveau, Set[Tuple[int, int]]]:
    """
...
def load_periodes_v2() -> List[Periode]:
    """
...
def load_all_data_v2(data_dir: Path) -> Dict:
    """
    Charge toutes les données (wrapper principal pour optimizer_V2)
    ...
    """
    logger.info("=" * 80)
    logger.info("CHARGEMENT DES DONNÉES (LOADERS V2 - Model V5_03_C)")
    logger.info("=" * 80)
    
    try:
        # Charger disciplines (hardcodées)
        disciplines = load_disciplines_v2()
        
        # Charger élèves
        eleves_path = data_dir / 'eleves_with_code.csv'
        eleves = load_eleves_v2(eleves_path)
        
        # Charger stages
        stages_path = data_dir / 'stages.csv'
        stages_lookup = load_stages_v2(stages_path)
        
        # Charger calendriers
        calendar_unavailability = load_calendars_v2(data_dir)
        
        # Charger périodes (hardcodées)
        periodes = load_periodes_v2()
