# =============================================================================
# MODÈLE V5_03_C2 - RÉSOLUTION SÉQUENTIELLE : POLYCLINIQUE EN PRIORITÉ
# =============================================================================
# Ce modèle implémente une approche de résolution EN DEUX PHASES :
# 
# PHASE 1 : Résolution uniquement pour la POLYCLINIQUE
#   - Crée un premier modèle avec seulement les affectations Polyclinique
#   - Optimise pour remplir tous les quotas Polyclinique
#   - Sauvegarde les affectations trouvées
#
# PHASE 2 : Résolution pour TOUTES LES AUTRES DISCIPLINES
#   - Fixe les affectations Polyclinique de la phase 1 comme contraintes
#   - Crée un modèle complet avec toutes les disciplines
#   - Optimise les autres disciplines SANS pouvoir modifier Polyclinique
#
# Résultat : Garantie matérielle que Polyclinique est placée en priorité.
# =============================================================================
...
# =============================================================================
# 2. DEFINITION DU MODELE OR-TOOLS - PHASE 1 : POLYCLINIQUE SEULEMENT
# =============================================================================
print("\n" + "="*80)
print("PHASE 1 : OPTIMISATION POLYCLINIQUE UNIQUEMENT")
print("="*80)

model_phase1 = cp_model.CpModel()
assignments_poly = {} # (id_eleve, id_discipline=1, index_vacation) -> BoolVar

print(f"Création des variables Polyclinique pour {len(eleves)} élèves sur {len(vacations)} créneaux...")

count_vars_poly = 0
for v_idx, vac in enumerate(vacations):
    slot_idx = vac.jour * 2 + (0 if vac.period == DemiJournee.matin else 1)
    
    # UNIQUEMENT POLYCLINIQUE
    disc = poly
    
    # Contrainte 3 (Fermeture Discipline)
    ...
    for el in eleves:
        ...
        # Contrainte 3 (Indisponibilité Élève)
        # 1. Cours
        ...
        if el.id_eleve in stages_eleves:
            for st in stages_eleves[el.id_eleve]:
                if st.debut <= vac.semaine <= st.fin:
                    is_in_stage = True
                    break
        if is_in_stage:
            continue

        # Création variable x_{e,poly,v}
        var_name = f"x_e{el.id_eleve}_d{disc.id_discipline}_v{v_idx}"
        assignments_poly[(el.id_eleve, disc.id_discipline, v_idx)] = model_phase1.NewBoolVar(var_name)
        count_vars_poly += 1

print(f"Modèle Phase 1 initialisé avec {count_vars_poly} variables.")

# =============================================================================
# 3. CONTRAINTES PHASE 1 - POLYCLINIQUE
# =============================================================================
print("Configuration contraintes Phase 1 (Polyclinique)...")

# Pré-calcul des groupements
vars_by_student_vac_p1 = collections.defaultdict(list)
vars_by_disc_vac_p1 = collections.defaultdict(list)
vars_by_student_disc_semaine_p1 = collections.defaultdict(list)

for (e_id, d_id, v_idx), var in assignments_poly.items():
    vars_by_student_vac_p1[(e_id, v_idx)].append(var)
    vars_by_disc_vac_p1[(d_id, v_idx)].append(var)
    semaine = vacations[v_idx].semaine
    vars_by_student_disc_semaine_p1[(e_id, d_id, semaine)].append((v_idx, var))

# Capacité
print("  - Capacité Polyclinique...")
for v_idx, vac in enumerate(vacations):
    slot_idx = vac.jour * 2 + (0 if vac.period == DemiJournee.matin else 1)
    vars_in_slot = vars_by_disc_vac_p1.get((poly.id_discipline, v_idx), [])
    if vars_in_slot:
        cap = poly.nb_eleve[slot_idx] if slot_idx < len(poly.nb_eleve) else 0
        model_phase1.Add(sum(vars_in_slot) <= cap)

# Unicité
print("  - Unicité...")
for v_idx in range(len(vacations)):
    for el in eleves:
        vars_for_student = vars_by_student_vac_p1.get((el.id_eleve, v_idx), [])
        if vars_for_student:
            model_phase1.Add(sum(vars_for_student) <= 1)

# Nb vacations par semaine (Polyclinique = 2)
print("  - Max 2 vacations/semaine Polyclinique...")
for el in eleves:
    if el.annee.value not in poly.annee: continue
    for s in range(1, 53):
        vars_entries = vars_by_student_disc_semaine_p1.get((el.id_eleve, poly.id_discipline, s), [])
        vars_week = [v[1] for v in vars_entries]
        if vars_week:
            model_phase1.Add(sum(vars_week) <= poly.nb_vacations_par_semaine)

# Paires de jours
print("  - Paires de jours Polyclinique...")
for el in eleves:
    if el.annee.value not in poly.annee: continue
    for s in range(1, 53):
        vars_entries = vars_by_student_disc_semaine_p1.get((el.id_eleve, poly.id_discipline, s), [])
        ...
        for (j1, j2) in poly.paire_jours:
            ...
            if vars_j1 and vars_j2:
                b_j1 = model_phase1.NewBoolVar(f"present_j{j1}_s{s}_e{el.id_eleve}_poly_p1")
                b_j2 = model_phase1.NewBoolVar(f"present_j{j2}_s{s}_e{el.id_eleve}_poly_p1")
                model_phase1.Add(sum(vars_j1) >= 1).OnlyEnforceIf(b_j1)
                model_phase1.Add(sum(vars_j1) == 0).OnlyEnforceIf(b_j1.Not())
                model_phase1.Add(sum(vars_j2) >= 1).OnlyEnforceIf(b_j2)
                
                pair_ok = model_phase1.NewBoolVar(f"pair_ok_{j1}_{j2}_{s}_e{el.id_eleve}_poly_p1")
                model_phase1.AddImplication(b_j1, b_j2).OnlyEnforceIf(pair_ok)

# Objectif Phase 1 : Maximiser quotas Polyclinique
print("Configuration objectif Phase 1...")
obj_terms_p1 = []
weights_p1 = []

vars_by_student_disc_all_p1 = collections.defaultdict(list)
for (e_id, d_id, _), var in assignments_poly.items():
    vars_by_student_disc_all_p1[(e_id, d_id)].append(var)

for el in eleves:
    if el.annee.value not in poly.annee: continue
    
    vars_list = vars_by_student_disc_all_p1.get((el.id_eleve, poly.id_discipline), [])
    if not vars_list: continue
    
    try:
        adx = poly.annee.index(el.annee.value)
        quota = poly.quota[adx]
    except ValueError:
        quota = 0
    ...
    if quota > 0:
        ...
        sat_var = model_phase1.NewIntVar(0, quota, f"sat_e{el.id_eleve}_poly_p1")
        model_phase1.Add(sat_var <= sum(vars_list))
        obj_terms_p1.append(sat_var)
        weights_p1.append(w_fill)
        
        is_success = model_phase1.NewBoolVar(f"success_e{el.id_eleve}_poly_p1")
        model_phase1.Add(sum(vars_list) >= quota).OnlyEnforceIf(is_success)
        model_phase1.Add(sum(vars_list) < quota).OnlyEnforceIf(is_success.Not())
        obj_terms_p1.append(is_success)
        weights_p1.append(w_success)

# Préférences jour
if poly.take_jour_pref:
    for k, var in assignments_poly.items():
        el = eleve_dict[k[0]]
        v_idx = k[2]
        vac = vacations[v_idx]
        if (vac.jour + 1) == el.jour_preference.value:
            obj_terms_p1.append(var)
            weights_p1.append(100)

model_phase1.Maximize(sum(t * w for t, w in zip(obj_terms_p1, weights_p1)))

# Résolution Phase 1
print("\n" + "="*80)
print("RÉSOLUTION PHASE 1 - POLYCLINIQUE")
print("="*80)

solver_p1 = cp_model.CpSolver()
solver_p1.parameters.max_time_in_seconds = args.time_limit if args.time_limit else 1800
solver_p1.parameters.log_search_progress = True
solver_p1.parameters.num_workers = 8

status_p1 = solver_p1.Solve(model_phase1)

# Sauvegarde des affectations Polyclinique
poly_assignments_fixed = {}  # (id_eleve, index_vacation) -> 1 si affecté
if status_p1 in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    print("\nPhase 1 terminée avec succès !")
    print(f"Score Phase 1 : {solver_p1.ObjectiveValue():,.0f}")
    
    for (e_id, d_id, v_idx), var in assignments_poly.items():
        if solver_p1.Value(var) == 1:
            poly_assignments_fixed[(e_id, v_idx)] = 1
    
    print(f"Affectations Polyclinique fixées : {len(poly_assignments_fixed)}")
else:
    print("ERREUR : Aucune solution trouvée en Phase 1. Arrêt.")
    sys.exit(1)

# =============================================================================
# 4. DEFINITION DU MODELE PHASE 2 - TOUTES LES DISCIPLINES
# =============================================================================
print("\n" + "="*80)
print("PHASE 2 : OPTIMISATION TOUTES DISCIPLINES (Polyclinique fixée)")
print("="*80)
...
for v_idx, vac in enumerate(vacations):
    ...
    for disc in disciplines:
        ...
        for el in eleves:
            ...
            # PHASE 2 : Si Polyclinique déjà affectée en Phase 1, on la fixe
            if disc.id_discipline == poly.id_discipline:
                if (el.id_eleve, v_idx) in poly_assignments_fixed:
                    # Créer variable et la fixer à 1
                    var_name = f"x_e{el.id_eleve}_d{disc.id_discipline}_v{v_idx}"
                    var = model.NewBoolVar(var_name)
                    model.Add(var == 1)  # FIXÉ À 1
                    assignments[(el.id_eleve, disc.id_discipline, v_idx)] = var
                    count_vars += 1
                else:
                    # Polyclinique NON affectée en Phase 1 -> interdire (fixer à 0)
                    var_name = f"x_e{el.id_eleve}_d{disc.id_discipline}_v{v_idx}"
                    var = model.NewBoolVar(var_name)
                    model.Add(var == 0)  # FIXÉ À 0
                    assignments[(el.id_eleve, disc.id_discipline, v_idx)] = var
                    count_vars += 1
            else:
                # Création variable normale pour autres disciplines
                var_name = f"x_e{el.id_eleve}_d{disc.id_discipline}_v{v_idx}"
                assignments[(el.id_eleve, disc.id_discipline, v_idx)] = model.NewBoolVar(var_name)
                count_vars += 1

print(f"Modèle Phase 2 initialisé avec {count_vars} variables (Polyclinique fixée).")

# =============================================================================
# 5. OPTIMISATION ET CONTRAINTES PHASE 2
# =============================================================================
...
# 6. OBJECTIFS D'OPTIMISATION PHASE 2
# OBJECTIF PRINCIPAL: Remplissage et Quotas (Polyclinique déjà fixée)
print("Configuration Objectifs Phase 2...")
...
for disc in disciplines:
    ...
    for el in eleves:
        ...
        if quota > 0:
            ...
            # MODÈLE C2: Augmentation DRASTIQUE pour Polyclinique - Priorité Absolue
            if disc.nom_discipline == "Polyclinique":
                w_fill = 10000     # 10x supérieur - Incitation maximale au remplissage
                w_excess = -15000  # Malus massif pour éviter tout dépassement
                w_success = 500000 # 50x supérieur - Priorité critique absolue pour quotas
    ...
    if discipline_success_vars:
        ...
        if disc.nom_discipline == "Polyclinique":
             w_grand_slam = 50000000 # 50 Millions points for Poly - MODÈLE C2: Priorité Absolue
...
# =============================================================================
# 7. RESOLUTION PHASE 2 ET EXPORT
# =============================================================================
print("\n" + "="*80)
print("RÉSOLUTION PHASE 2 - TOUTES DISCIPLINES")
print("="*80)
...
if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    ...
    print("\n" + "="*80)
    print("RÉSULTAT FINAL - MODÈLE C2 (RÉSOLUTION SÉQUENTIELLE)")
    print("="*80)
    print(f"Phase 1 (Polyclinique) : {len(poly_assignments_fixed)} affectations")
    print(f"Phase 2 (Toutes disciplines) terminée")
    print(f"Score brut Phase 2 : {raw_score:,.0f}")
